    ...
    $routeProvider
        ...
        .when('/client/:id', {
            ...
            onEnter       : ['$location', function($location) {
                // Remove token parameter from URL to prevent it from being exposed
                console.log('[Route] 进入 /client/:id，开始清理 token 参数');
                var search = $location.search();
                if (search.token) {
                    console.log('[Route] 检测到 token 参数，已移除');
                    delete search.token;
                    $location.search(search);
                } else {
                    console.log('[Route] 未检测到 token 参数');
                }
                var search = $location.search();
                if (search.token) {
                    delete search.token;
