...
angular.module('auth').factory('authenticationService', ['$injector',
    ...
    /**
     * The unique identifier of the storage key which stores the latest
    ...
    /**
     * 🔥 Multi-tab support: Use sessionStorage instead of localStorage
     * to allow independent sessions in different browser tabs.
     * Each tab will have its own auth token and won't interfere with others.
     */
    ...
    var setAuthenticationResult = function setAuthenticationResult(data) {
        ...
        if (!data) {
            cachedResult = null;
            // 🔥 Multi-tab support: use sessionStorage for tab-independent sessions
        ...
        // Note that only the auth token is stored in persistent storage.
        ...
        // calling getCurrentToken().
            ...
            // 🔥 Multi-tab support: use sessionStorage for tab-independent sessions
            // Persist only the auth token, and only if not anonymous
            if (data.username !== AuthenticationResult.ANONYMOUS_USERNAME) {
                if (useSessionStorage) {
                    sessionStorage.setItem(AUTH_TOKEN_STORAGE_KEY, data.authToken);
                } else {
        ...
        return parameters.then(function requestParametersReady(requestParams) {
            ...
            .then(function authenticationSuccessful(data) {
                ...
                if (data.authToken !== currentToken) {
                    ...
                    // 🔥 Multi-tab support: When using sessionStorage, each tab has its own
                    // token storage, so we should NOT revoke the "old" token because it
                    // belongs to THIS tab only (or doesn't exist yet).
                    // Only revoke if using shared localStorage (legacy behavior).
                    if (currentToken && !useSessionStorage) {
    ...
    service.getCurrentToken = function getCurrentToken() {
        ...
        // 🔥 Multi-tab support: use sessionStorage for tab-independent sessions
        // Fall back to the value from storage if not found in cache
        if (useSessionStorage) {
